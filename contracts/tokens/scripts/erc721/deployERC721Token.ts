...
import { ERC721Token, ERC721Token__factory } from "../../typechain";
import { uploadMetadata } from "../uploadMetadata";
import { mintERC721Token } from "./mintERC721Token";

const deployERC721Token = async (): Promise<string> => {
  const [owner, alice] = await hre.ethers.getSigners();

  const ERC721Token = <ERC721Token__factory>(
  ...
  const erc721Token: ERC721Token = await ERC721Token.deploy(
    input.name,
    input.symbol,
    contractURI,
    tokenURI,
  ...
  await erc721Token.deployTransaction.wait(5);

  console.log("ERC721Token deployed");
  ...
  await mintERC721Token(erc721Token, owner, alice, alice.address);
