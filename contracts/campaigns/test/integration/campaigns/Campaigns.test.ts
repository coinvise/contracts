...
import { integrationFixtureCampaigns } from "../../shared/fixtures";
import { BASIS_POINTS, CLAIM_FEE, CREATOR_SHARE, PLATFORM_SHARE, REFERRER_SHARE } from "../../shared/utils";

export function integrationTestCampaigns(): void {
  describe("Campaigns", function () {
    beforeEach(async function () {
      const { campaigns, erc20Token, protocolRewards } = await this.loadFixture(integrationFixtureCampaigns);
    ...
    describe("createCampaign()", function () {
      it("should transfer total required tokens if not ETHAddress", async function () {
        ...
        await this.contracts.erc20Token
          ...
          .approve(this.contracts.campaigns.address, totalAmount);
        ...
        await expect(
          ...
          [this.signers.bob, this.contracts.campaigns],
      ...
      it("Should allow to createCampaign() multiple times and each should have separate valid states", async function () {
        ...
        campaign = await this.contracts.campaigns.campaigns(this.signers.bob.address, 1);
        expect(campaign.tokenAddress).to.eq(AddressZero);
        expect(campaign.maxClaims).to.eq(0);
        expect(campaign.noOfClaims).to.eq(0);
        expect(campaign.amountPerClaim).to.eq(0);
        ...
        campaign = await this.contracts.campaigns.campaigns(this.signers.carol.address, 0);
        expect(campaign.tokenAddress).to.eq(AddressZero);
        expect(campaign.maxClaims).to.eq(0);
        expect(campaign.noOfClaims).to.eq(0);
        expect(campaign.amountPerClaim).to.eq(0);
        ...
        await erc20TokenA.connect(this.signers.bob).approve(this.contracts.campaigns.address, totalAmount);
        const createCampaignBob0 = await this.contracts.campaigns
          .connect(this.signers.bob)
          .createCampaign(erc20TokenA.address, 3, parseEther("3"), 0);
        receipt = await createCampaignBob0.wait();
        campaignId = getArgFromEvent(
          this.contracts.campaigns,
          receipt,
          this.contracts.campaigns.interface.events["CampaignCreated(address,uint256)"].name,
          "campaignId",
        );
        ...
        await erc20TokenB.connect(this.signers.bob).approve(this.contracts.campaigns.address, totalAmount);
        const createCampaignBob1 = await this.contracts.campaigns
          .connect(this.signers.bob)
          .createCampaign(erc20TokenB.address, 4, parseEther("4"), 0);
        ...
        await erc20TokenC.connect(this.signers.carol).approve(this.contracts.campaigns.address, totalAmount);
        const createCampaignCarol0 = await this.contracts.campaigns
          .connect(this.signers.carol)
          .createCampaign(erc20TokenC.address, 5, parseEther("5"), 0);
        ...
        await this.contracts.campaigns
          .connect(this.signers.carol)
          .claim(this.signers.bob.address, 1, r, s, v, ethers.constants.AddressZero, { value: CLAIM_FEE });
        campaign = await this.contracts.campaigns.campaigns(this.signers.bob.address, 1);
        expect(campaign.noOfClaims).to.eq(1);
      ...
      describe("Protocol Rewards", function () {
        ...
        it("Should emit ClaimFeePaid() on claim()", async function () {
          ...
          await expect(
            ...
            .to.emit(this.contracts.campaigns, "ClaimFeePaid")
            .withArgs(
              CLAIM_FEE,
              this.signers.carol.address,
        ...
        it("Should not split rewards if sponsored claim", async function () {
          ...
          await this.contracts.campaigns
            .connect(this.signers.bob)
            .createCampaign(this.contracts.erc20Token.address, 10, parseEther("10"), 1);
    ...
    describe("withdrawCampaign()", function () {
      it("should transfer remaining tokens if not ETHAddress", async function () {
        ...
        await expect(this.contracts.campaigns.connect(this.signers.bob).withdrawCampaign(0)).to.changeTokenBalances(
          this.contracts.erc20Token,
